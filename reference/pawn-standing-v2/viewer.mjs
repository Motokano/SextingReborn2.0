...
for(const p of bind.pieces){const [x,y,w,h]=p.box;ctx.save();const dx=p.id==='base'?0:(x+w/2-607.5)*split*.007,dy=p.id==='base'?split*1.1:(y+h/2-647.5)*split*.003;ctx.translate(x+dx+w*p.pivot[0],y+dy+h*p.pivot[1]);ctx.rotate(p.angleDeg*Math.PI/180);ctx.imageSmoothingQuality='high';const c=p.clip||[0,0,1,1];ctx.beginPath();ctx.rect((c[0]-p.pivot[0])*w,(c[1]-p.pivot[1])*h,(c[2]-c[0])*w,(c[3]-c[1])*h);ctx.clip();ctx.drawImage(sheet,...p.crop,-w*p.pivot[0],-h*p.pivot[1],w,h);ctx.restore()}
