...
- **装备/载具配置提示**：设计**衣服**时在配置中声明口袋格数（如 2～4）；设计**背心/弹挂**时声明背心格数；设计**背包**时声明背包格数及重量折扣系数（或从全局表读）；设计**载具**时声明载重上限 \(W_{\text{载具,max}}\) 及拖拽时施加给玩家的 debuff（如移速、体力消耗系数等）。上述 tick 成本与重量系数建议从**常数表**读取，便于调参。
...
- **存档顶层结构（推荐）**：解密后的存档 JSON 建议采用以下顶层 key；具体嵌套由实现定，可增删字段。
  ...
  - `equipment`：当前装备，按槽位（如左手、右手、手套、衣服、背心、背包等）存储装备引用或 ID。
