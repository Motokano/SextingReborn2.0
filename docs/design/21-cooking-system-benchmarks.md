...
2026-09-12 面板调整：桌面使用双栏，左侧切换「背包食材／已学菜谱」，右侧选择做法并调整本次食材份数。底部固定显示消耗、开工障碍与制作进度。「锅具与补给」和帮助默认收起，缺水、燃料或锅具时提供展开入口；主灶接通水源后隐藏添水列表，临时灶仍检查水量。材料、锅具不附带内部编号，帮助使用日常操作说明。窄屏上下排列，操作按钮常驻。

浏览器回归：`tools/test-cooking-panel.cjs` 使用真实 HTML、样式、面板及站点模块，隔离库存和制作调用，覆盖份数编辑、菜谱填料、补给入口、主灶与临时灶的水量差异、体力不足、进度和390px布局。运行需环境提供 Playwright。
