> 历史归档：不得作为当前部署规范。当前入口为[制药系统](47-pharmacy-system.md)。

# 四十七、制药系统（设计定稿）

> 经验规则已确认：[普通加工失败获得25%经验](47-pharmacy-crafting-experience.md)。配伍失败不给经验，无越级倍率；待实装。

> 加工等级规则已更新：[推荐等级与良品率](47-pharmacy-crafting-levels.md)。低等级可尝试，成功率受罚；替代旧硬门槛设计。具体曲线待定，未实装。

> 发布门槛已确认：[无实际作用的药品分支暂缓](47-pharmacy-functional-release.md)。止血/复苏消费者缺口已核对，实际停用或补接待部署。

> 解毒用途已确认：[基础口服解毒药](47-pharmacy-detox.md)。加快体内毒性代谢，与配药减毒分开；具体物品与数值待定，未实装。

> 最新配置约束已确认：[有效药效开工条件与取消历史记录](47-pharmacy-compounding-failure.md)。无实际药效不开工；移除历史/回填及历史信息豁免，保留当次分级提示。待实装，覆盖旧历史功能设计。

> 配伍失败处理已更新：[统一药渣与避免失败变体物品](47-pharmacy-compounding-failure.md)。替代旧沉淀注射液产物方案，合法高毒配方仍可制作；待实装。

> 注射液身份已确认：[按完整配方判断同一种药](47-pharmacy-compound-identity.md)。相同配方刷新、不同配方独立计时，堆叠兼容性另判；待接入。

> 药效混用规则已确认：[同类效果取最高与独立计时](47-pharmacy-effect-stacking.md)。不同效果并存，恢复按具体状态、外敷按部位分别判断；待统一实装。

> 续药规则已确认：[重复用药与持续时间刷新](47-pharmacy-redosing.md)。再次成功服用刷新窗口，每次依赖与毒性照常增加；尚未实装。
...
> 成瘾规则更新：[统一依赖与分阶段戒断缓解](47-pharmacy-addiction-rework.md)已获确认，尚未实装。替代旧版任意入体药按强度完全压制惩罚的设计；具体药物数值待定。
...
> **当前完整制药表**：[可搜索表与制作路线](../reference/pharmacy-current.html) / [Markdown 全表](../reference/pharmacy-current.md)。从运行数据生成，含 68 条配方、30 条成品完整路线及动态配药；路线成本计入单次产量与剩余中间品，优先于本文历史人工成本快照。更新命令：`node tools/export-pharmacy-reference.mjs`。

> **注射窗口新定稿（2026-09-10）**：见[按药效族设置注射窗口](47-injection-effect-windows.md)。该规则替代下文历史快照中的统一注射 10 tick；非注射时序不变，逐 tick 恢复按原总预算分摊。

> **2026-09-10 全目录重整入口**：[药物种类、药效与配方重整方案](47-pharmacy-rework.md)及[全量逐项迁移附表](47-pharmacy-rework-inventory.md)。新方案目前是设计稿，未改运行时；本文下方历史“全量落地”和旧数量不能代替现状审计。当前源表为 96 件物品、30 件固定成品、68 条配方；止血与复苏仍有消费者缺口。
...
|---|---|---|---|---|---|---|
| 注射 inject | 0 tick（立即） | 按族 20–50；储备模板 10，见[窗口定稿](47-injection-effect-windows.md) | 快速起效，各族独立结束；恢复按总预算分摊 | 全身直达 | 持续加成保留 1.0 基准；恢复速率另按预算 | 保留原负担与针具卫生 |
...
- 生效时间 = 使用后到 buff 开始生效的 tick 数；持续时间 = buff 的 `durationTicks`。剂型给出默认值，**注射由药效族覆盖持续窗口**，不能再以统一 10 tick 推断所有针剂。
...
- 门禁已实现（2026-09）：**外敷/注射战斗中不可**（读取 [CombatEngagement 统一交战状态](07-combat-engagement.md)，不再按两格距离判断）、**注射需静止**（不在挂机采集/调息/休息/制作中）；口服/吸入不受该战斗门禁，仍需满足一般行动条件。
...
### 6.6 工序链总表（原料 → 成品，2026-09 实测）

> 由 `data/recipes.json` + `data/recipe-methods.json` 递归展开生成（固定配方链；配药台动态链见末行）。
> **工序数** = 该成品链上最长路径的配方执行次数；**工序次数** = 含中间品份数倍率的实际执行次数。

| 成品 | 途径 | 族·档位 | 工序数 | 完整链（原料 →(工艺) … →(工艺) 成品） |
|---|---|---|---|---|
| 三七止血散 | topical | coagulant·regular | 2 | 三七 →(粉碎) 三七粉 →(调和) 三七止血散 |
| 安神汤 | drink | sedative·regular | 2 | 大麻 →(粉碎) THC粉 + 纯净水 →(浸渍) 安神汤 |
| 安神药烟 | inhale | sedative·regular | 2 | 大麻 →(粉碎) THC粉 + 烟草 →(卷制) 安神药烟 |
| 幻梦汤 | drink | hallucinogen·regular | 2 | 裸盖菇 →(粉碎) 裸盖菇素粉 + 纯净水 →(浸渍) 幻梦汤 |
| 幻梦烟 | inhale | hallucinogen·potent | 2 | 裸盖菇 →(粉碎) 裸盖菇素粉×2 + 烟草 →(卷制) 幻梦烟 |
| 强心注射液 | inject | revive·regular | 2 | 猪心 + 纯净水 →(浸渍) 强心粉 + 生理盐水 →(浸渍) 强心注射液 |
| 提神汤液 | drink | stimulant·weak | 2 | 赤花藤 →(粉碎) 赤花藤粉 + 纯净水 →(浸渍) 提神汤液 |
| 提神注射液 | inject | stimulant·potent | 2 | 麻黄 →(粉碎) 麻黄碱粉 + 生理盐水 →(浸渍) 提神注射液 |
| 提神药烟 | inhale | stimulant·regular | 2 | 烟草 →(粉碎) 尼古丁粉 + 烟草 →(卷制) 提神药烟 |
| 止血注射液 | inject | coagulant·regular | 2 | 三七 →(粉碎) 三七粉 + 生理盐水 →(浸渍) 止血注射液 |
| 活络药膏 | topical | mobility·regular | 2 | 苦根草 →(粉碎) 苦根草粉 + 猪油 →(调和) 活络药膏 |
| 浓煎提神汤 | drink | stimulant·potent | 2 | 赤花藤 →(粉碎) 赤花藤粉×2 + 纯净水 →(浸渍) 浓煎提神汤 |
| 白芨止血膏 | topical | coagulant·potent | 2 | 白芨 →(粉碎) 白芨粉 + 猪油 →(调和) 白芨止血膏 |
| 红花活络膏 | topical | mobility·potent | 2 | 红花 →(粉碎) 红花粉×2 + 猪油 →(调和) 红花活络膏 |
| 红花药泥 | topical | mobility·regular | 2 | 红花 →(粉碎) 红花粉 + 纯净水 →(调和) 红花药泥 |
| 续力注射液 | inject | strength·potent | 2 | 鹿茸 →(粉碎) 鹿茸粉 + 生理盐水 →(浸渍) 续力注射液 |
| 续力药汤 | drink | strength·potent | 2 | 鹿茸 →(粉碎) 鹿茸粉 + 纯净水 →(浸渍) 续力药汤 |
| 续航糖浆 | drink | energy·regular | 2 | 甘蔗 + 纯净水 →(浸渍) 葡萄糖粉 + 纯净水 →(浸渍) 续航糖浆 |
| 补液汤 | drink | rehydrate·potent | 2 | 精海盐 + 纯净水 →(浸渍) 补液粉 + 纯净水 →(浸渍) 补液汤 |
| 补液注射液 | inject | rehydrate·regular | 2 | 精海盐 + 纯净水 →(浸渍) 补液粉 + 生理盐水 →(浸渍) 补液注射液 |
| 醒神注射液 | inject | antistun·regular | 2 | 石菖蒲 →(粉碎) 石菖蒲粉 + 生理盐水 →(浸渍) 醒神注射液 |
| 醒神茶 | drink | antistun·regular | 2 | 石菖蒲 →(粉碎) 石菖蒲粉 + 纯净水 →(浸渍) 醒神茶 |
| 镇痛注射液 | inject | analgesic·potent | 2 | 罂粟 →(粉碎) 吗啡粉 + 生理盐水 →(浸渍) 镇痛注射液 |
...
| 工艺 | 配件 | 燃料 | 体力 | tick | 解锁 |
|---|---|---|---|---|---|
| 粉碎 | tool_rolling_pin_pharmacy | 12 | 2 | 1 | — |
| 浸渍 | tool_strainer_pot_pharmacy | 10 | 2 | 2 | — |
| 蒸馏 | tool_distiller_pharmacy | 18 | 3 | 3 | 制药 6 级 |
| 过滤 | tool_layer_filter_sheet_pharmacy | 8 | 2 | 2 | 制药 4 级 |
| 离心 | tool_long_handle_flask_pharmacy | 14 | 3 | 2 | 制药 8 级 |
| 结晶 | tool_steel_basin_pharmacy | 16 | 2 | 3 | 制药 10 级 |
| 压片 | tool_garlic_press_pharmacy | 10 | 2 | 1 | — |
| 调和 | tool_mortar_pharmacy | 8 | 2 | 2 | — |
| 卷制 | tool_roller_pharmacy | 6 | 2 | 1 | — |

**成品总消耗（沿链累加）**

| 成品 | 工序次数 | 燃料 | 体力 | tick | 工艺组合 |
|---|---|---|---|---|---|
| 安神药烟 | 2 | 18 | 4 | 2 | 卷制×1 + 粉碎×1 |
| 提神药烟 | 2 | 18 | 4 | 2 | 卷制×1 + 粉碎×1 |
| 三七止血散 | 2 | 20 | 4 | 3 | 调和×1 + 粉碎×1 |
| 强心注射液 | 2 | 20 | 4 | 4 | 浸渍×2 |
| 活络药膏 | 2 | 20 | 4 | 3 | 调和×1 + 粉碎×1 |
| 白芨止血膏 | 2 | 20 | 4 | 3 | 调和×1 + 粉碎×1 |
| 红花药泥 | 2 | 20 | 4 | 3 | 调和×1 + 粉碎×1 |
| 续航糖浆 | 2 | 20 | 4 | 4 | 浸渍×2 |
| 补液汤 | 2 | 20 | 4 | 4 | 浸渍×2 |
| 补液注射液 | 2 | 20 | 4 | 4 | 浸渍×2 |
| 安神汤 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 幻梦汤 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 提神汤液 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 提神注射液 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 止血注射液 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 续力注射液 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 续力药汤 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 醒神注射液 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 醒神茶 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 镇痛注射液 | 2 | 22 | 4 | 3 | 浸渍×1 + 粉碎×1 |
| 镇痛药丸 | 2 | 22 | 4 | 2 | 压片×1 + 粉碎×1 |
| 幻梦烟 | 3 | 30 | 6 | 3 | 卷制×1 + 粉碎×2 |
| 红花活络膏 | 3 | 32 | 6 | 4 | 调和×1 + 粉碎×2 |
...
> **信息显示后续方案（2026-09-10，设计提案，未实装）**：见 [制药信息清单、显示优先级与解锁阶段](47-pharmacy-information-display.md)。该补充逐项区分操作反馈、成品药效、原料分析和配比解释；下文“已实现”仍是旧实现快照，不代表新阶段已经落地。药名与描述还受语言等级控制，不能将“不设制药技能门”理解为绕过语言门槛。
...
- 多 buff 叠加/时长结算：**各成分按各自药效族的剂型 buff 独立计时**（同族合成单档、跨族并集）；注射改按[药效族窗口定稿](47-injection-effect-windows.md)（20–50 tick，储备模板 10）；其余保留吸入 18 / 口服 60 / 外敷 90 tick，生效时间 `onsetTicks` 由 buff-system 统一门闸。
...
| 续力（携行） | 鹿茸 | 狩猎/地牢高层 | 直接提高负重上限；强效口服 +6 kg / 180 tick，强效注射 +7.5 kg / 120 tick；同类取最高值，不增加筋骨 |
