...
> **⚠️ 废弃状态：本设计已被 [34-muscle-system-rework.md](34-muscle-system-rework.md)（肌肉系统）取代。**
> 穴位 → 肌肉（22 肌群 × 478 块）；任脉/督脉/全通成就 → 肌群全通解锁大型被动容量；`docs/穴位.csv` 已删除，数据由 `tools/build_muscles.js` 生成 `data/muscles.json`。
> 本文档仅保留作**历史参考**，实现与配置一律以 `34` 与运行时（`js/muscles.js`）为准。

本模块（旧设计）定义**经脉与穴位系统**，用于在不改变核心战斗流程的前提下，为角色提供一条长期、稳定的数值成长线：
