...
> **2026-09-12 来源调整（已定，待实现）**：基础采集也集中到新手地牢与野外随机单层地牢，露天地表基本不再采集；这些浅层地牢提供少量基础材料，主题地牢承担大部分采集产量和效率。下文“地表池”改按**基础材料池**理解，不再表示露天固定点持续供应；原材料身份清单可作迁移基础，具体池、权重、产量与地区标签映射待调整。采集点改为有限可采次数，规则见 `11-skills.md` §8.2.2。此调整针对采集，不改变种植、畜牧本身的产出归属；文末已有数据实现记录不代表此次调整已落地。
