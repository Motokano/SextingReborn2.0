...
> **2026-09-11 校准已接入运行时**：35 种现有食物采用独立份量消化、随消化经验、长期比例营养与跨餐增减重。完整生效数值、参考流程及限制见 [食物校准表](../reference/food-balance-calibration.md)。本文下方旧档位表、直接营养恢复、消化 buff 载体及“待实现”描述为重审历史，冲突处以校准表及 `data/food-balance.json` / `survival-config.food_model` 为准。饭盒容器功能另行实现。
...
- **一餐的饱食预算**：必须扣除消化期间的基础衰减。以同时开始消化、各持续 45 tick、合计约 96 饱食的三道家常为例，完整消化净恢复约 `96 − 45 = 51`，从 20 升至约 71，并非接近满值。此例忽略逐道食用耗时、运行时小数取整及其他状态修正；实际配餐按各菜的消化时长分段计算。（2026-09-11 校正算术，未调整菜品数值。）
...
> **2026-09-11 重审决定 · 延迟吃撑与重复进食（优先于旧同菜规则）**：慢消化与延迟吃撑是有意保留的玩法。短时间连续进食后，食物逐渐释放饱食，角色可能在之后才进入过量区间；不新增用来立即限制进食的“胃内负担”系统。**每次进食均保留实际摄入量，同菜重复食用也累积待消化量，不能通过只刷新消化时长抹去已摄入食物。** 消化与特殊增益分开处理：同菜的特殊增益只刷新时长、不叠强度；普通食物与饭盒逐菜食用使用同一规则。重复份量采用并行消化、排队消化还是限速消化，以及对应经验入池时机，仍待确定；本次仅确认摄入量守恒和延迟后果，尚未修改运行时。

```
当前饱食每 tick 变化 = -1（基础衰减） + Σ(每份活性消化食物的每 tick 贡献 c)
```

- **2026-09-11 已定：每份独立、同时消化**。每次吃下的一份食物建立独立消化记录，持续该份的 `消化时长` tick，期间每 tick 贡献 `c`；同菜与异菜一视同仁，贡献相加，不排队、不新增总消化速率上限。
- 新吃的一份从自身食用时点开始消化，不覆盖旧份量，也不刷新旧份量的剩余时间。每份完整消化释放自身总量（生存数值上限及过量状态修正仍按对应规则结算）。
- 某份消化结束 → 仅该份退出，变化率回落；其他份量继续按自身进度消化。同菜特殊增益仍只刷新时长、不叠强度，不随消化份数倍增。
- 上述规则取代本节重审说明中的“消化调度待定”。**经验入池时机亦已于 2026-09-11 确定**：每份按实际消化进度逐步入池，使用发放时的长期营养倍率，睡眠沿用现有成长结算；不在食用瞬间发放整份经验。详见 `48-meal-balance-attribute-exp.md` §四重审决定，运行时尚未迁移。
...
> **2026-09-11 重审口径**：营养代表长期饮食状况，连续合理搭配的普通家常菜也能达到最高营养档；高级料理通过特殊效果和经验量体现价值，不靠料理档位提高营养收益。下文快速补营养与按菜品档位增加营养供给的旧数值意图已废止，替代公式待定，详见 `48-meal-balance-attribute-exp.md` 顶部重审决定。
...
> **2026-09-11 重审决定 · 跨餐累积增重**：体重是过量进食的长期代价，不能在饱食回到正常时清空此前积累。下文连续 144 tick 增重及沿用连续计时的口径待替换，详见 `25-height-bmi.md` §25.10 重审决定；具体累积公式待定。吃撑导致经验或营养吸收损失尚未获确认，不作为已定规则。
